...
def test_greet():
    response = client.post("/greet", json={"name": "Peace"})
    assert response.status_code == 200
    assert response.json()["message"] == "hello, Peace!"

def test_salute():
    response = client.post("/salute", json={"name": "Peace"})
    assert response.status_code == 200
    assert "captain" in response.json()["message"]

def test_passenger_summary_valid():
    response = client.post("/passenger-summary", json={"age": 29, "fare": 100, "pclass": 1})
    assert response.status_code == 200
    assert response.json()["pclass"] == 1

def test_passenger_summary_rejects_invalid_pclass():
    response = client.post("/passenger-summary", json={"age": 29, "fare": 100, "pclass": 99})
